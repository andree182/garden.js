   ...
   // Trunk Properties
    ...
    useLayoutEffect(() => {
        ...
        for (let i = 0; i < branchCount; i++) {
            ...
            const yStart = branchStartY + random() * branchHeightRange;
            // Slight offset from center for start point (less for thinner trunks)
            const startOffsetRadius = currentTrunkDiameter * 0.1 * random();
            ...
            const phi = Math.acos(2 * random() - 1); // Elevation angle
            const theta = random() * Math.PI * 2;   // Azimuth angle
            ...
            const branchLength = currentBranchLengthMax * (0.6 + random() * 0.4);
            ...
            const rollAngle = random() * Math.PI * 0.2 - Math.PI * 0.1;
    ...
    useLayoutEffect(() => { // Fruit position effect
        ...
        for (let i = 0; i < fruitCount; i++) {
            ...
            const phi = Math.acos(2 * random() - 1); const theta = random() * Math.PI * 2;
            // Point on surface
            ...
            const zSurf = rSurfaceXZ * Math.sin(phi) * Math.sin(theta);
             ...
             const offsetFactor = 1.0 + (random() * 0.2 - 0.1); // e.g., 0.9 to 1.1
